...
//MODIFY------------------------
		$result = db_query("SELECT id,update_interval,auth_login,
			...
			pubsub_state, auth_pass_encrypted, update_method,
...
//NEW------------------------------
			if ($update_method == 258)
				db_query("DELETE FROM ttrss_user_entries WHERE feed_id = '$feed'");
//---------------------------------
...
//				$entry_title = decode_numeric_entities($entry_title);
	...
	//				if ($_REQUEST["xdebug"]) print "$query\n";
					...
					if (db_num_rows($result) == 0) {
						...
						if ($update_method != 258 && $score >= -500 && !find_article_filter($article_filters, 'catchup')) {
							$unread = 'true';
